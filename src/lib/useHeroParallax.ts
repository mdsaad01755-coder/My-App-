 ...
 * fade only, no blob parallax) on low-powered devices to keep scroll
 * buttery on weak hardware.
 */
  ...
  useEffect(() => {
    ...
    function build() {
      ...
      ctx = gsap.context(() => {
        ...
        // Blob parallax: only on full-tier devices — this is the most
        // expensive part (two continuously-transformed large blurred
        // layers), so low-tier devices skip it entirely.
        if (tier === "full") {
          if (blobA.current) {
